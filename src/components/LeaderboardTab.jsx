...
import { RelPar, countHIO, isRoundSealed, computeAchievements, effectiveMatchType, isLeagueRound, isRoundHiddenForDisplay, championshipFingerprints, roundHoleCount } from "../utils/helpers.jsx";

const PUTTS_MIN_ROUNDS = 3;

export default function LeaderboardTab({ me, playerStats, rounds, deleteRoundFromDB, leagueMatches, openRoundDetail, openPlayerProfile, allCourses, lbHoleFilter, setLbHoleFilter, statsMode, setStatsMode, lvlFilter, setLvlFilter }) {
  ...
  const getAvgPutts = (name) => {
    const pr = rounds.filter(r => r.player === name && r.totalPutts != null && roundHoleCount(r)===lbHoleFilter && !isRoundSealed(r, leagueMatches, me) && modeMatch(r) && lvlMatch(r));
    if (pr.length < PUTTS_MIN_ROUNDS) return null;
    return Math.round(pr.reduce((s, r) => s + r.totalPutts, 0) / pr.length * 10) / 10;
  };
  // Hide the Avg Putts column entirely if no player has enough data — Quick
  // Score rounds don't track putts so a half-empty column looks broken.
  const showPuttsColumn = playerStats.some(p => getAvgPutts(p.name) != null);

  const filteredRounds = rounds.filter(r => roundHoleCount(r)===lbHoleFilter && modeMatch(r) && lvlMatch(r));
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* Player Rankings */}
      <div style={{background:C.card,borderRadius:12,border:`1px solid ${C.border}`,overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:440}}><thead><tr style={{background:C.accent}}><th style={{padding:"8px 6px",textAlign:"left"}}>#</th><th style={{padding:"8px 4px",textAlign:"left"}}>Player</th><th style={{padding:"8px 4px",textAlign:"center"}}>HDCP</th><th style={{padding:"8px 4px",textAlign:"center"}}>Rnds</th><th style={{padding:"8px 4px",textAlign:"center"}}>Best</th><th style={{padding:"8px 4px",textAlign:"center"}}>Avg</th><th style={{padding:"8px 4px",textAlign:"center"}}>🎯</th>{showPuttsColumn&&<th style={{padding:"8px 4px",textAlign:"center",color:C.blue}}>Putts</th>}</tr></thead><tbody>{playerStats.map((p,i)=>{
        const avgPutts = getAvgPutts(p.name);
        return (<tr key={p.name} style={{borderTop:`1px solid ${C.border}`,background:p.name===me?"rgba(74,170,74,0.06)":"transparent"}}><td style={{padding:"8px 6px",fontWeight:700,color:i===0?C.gold:i<3?C.greenLt:C.muted}}>{i+1}</td><td style={{padding:"8px 4px",fontWeight:600,cursor:"pointer",color:p.name===me?C.greenLt:C.text}} onClick={()=>openPlayerProfile(p.name)}>{p.name}</td><td style={{padding:"8px 4px",textAlign:"center",fontWeight:700,color:p.handicap!=null&&p.handicap<0?C.greenLt:p.handicap!=null&&p.handicap>0?C.red:C.text}}>{p.handicap!=null?p.handicap:"—"}</td><td style={{padding:"8px 4px",textAlign:"center",color:C.muted}}>{p.rounds}</td><td style={{padding:"8px 4px",textAlign:"center"}}>{p.best??"-"}</td><td style={{padding:"8px 4px",textAlign:"center"}}>{p.avg??"-"}</td><td style={{padding:"8px 4px",textAlign:"center",color:"#ff6b00"}}>{p.holeInOnes||0}</td>{showPuttsColumn&&<td style={{padding:"8px 4px",textAlign:"center",color:C.blue}}>{avgPutts??"-"}</td>}</tr>);
      })}</tbody></table></div></div>

      {/* All Rounds */}
      <div style={{background:C.card,borderRadius:12,padding:14,border:`1px solid ${C.border}`}}><div style={{fontWeight:600,marginBottom:8}}>All Rounds ({lbHoleFilter}H)</div>{filteredRounds.map(r=>{const hio=r.holeInOnes||countHIO(r.scores)||0;const sealed=isRoundSealed(r,leagueMatches,me);const hidden=isRoundHiddenForDisplay(r,leagueMatches,rounds,me,champFps);const hc=roundHoleCount(r);const lvl=r.courseLevel;const lvlColor=lvl==="Easy"?"#22c55e":lvl==="Medium"?"#3b82f6":lvl==="Hard"?"#f59e0b":"#ef4444";const mt=effectiveMatchType(r,champFps);const matchTag=mt==="championship"?"🏆":mt==="playoff"?"⚡PO":mt==="league"?"⚡":mt==="pga"?"📺":null;const matchColor=mt==="championship"?C.gold:mt==="pga"?C.blue:C.greenLt;return<div key={r.id} onClick={()=>openRoundDetail(r)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:`1px solid ${C.border}`,fontSize:12,cursor:"pointer"}}><div><span style={{fontWeight:600}}>{r.player}</span><span style={{color:C.muted,fontSize:10,marginLeft:6}}>{r.course}</span>{lvl&&<span style={{fontSize:8,fontWeight:700,color:lvlColor,marginLeft:4}}>{lvl}</span>}{matchTag&&<span style={{fontSize:8,color:matchColor,marginLeft:3}}>{matchTag}</span>}{r.par3&&<span style={{fontSize:8,color:"#b48af8",marginLeft:3,fontWeight:700}}>P3</span>}{hc===9&&<span style={{color:C.blue,fontSize:9,marginLeft:4}}>9H</span>}<span style={{color:C.muted,fontSize:10,marginLeft:6}}>{r.date}</span></div><div style={{display:"flex",gap:6,alignItems:"center"}}>{sealed?<span style={{color:C.muted,fontSize:11}}>🔒 Sealed</span>:hidden?<span style={{color:C.muted,fontSize:11}}>🙈</span>:<><span style={{fontWeight:700}}>{r.total}</span><RelPar s={r.total} p={r.par}/></>}{!sealed&&hio>0&&<span style={{fontSize:10,color:"#ff6b00"}}>🎯{hio}</span>}<button onClick={e=>{e.stopPropagation();if(confirm(`Delete ${r.player}'s round?`))deleteRoundFromDB(r.id);}} style={{background:"transparent",border:"none",color:C.muted,cursor:"pointer",fontSize:10,opacity:0.5}}>🗑</button></div></div>;})}</div>
