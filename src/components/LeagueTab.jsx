...
export default function LeagueTab({
  ...
  const Active = () => {
    ...
    return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* Schedule */}
      {leagueView==="schedule"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>{Array.from({length:totalRds},(_,ri)=>{const rd=ri+1;const rm=regMatches.filter(m=>m.round===rd);if(!rm.length)return null;return(<div key={rd}><div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:6}}>Round {rd}</div>{rm.map(m=>{const my=m.player1===me||m.player2===me;return(<div key={m.id} style={{background:my?"rgba(74,170,74,0.06)":C.card,borderRadius:8,padding:"10px 12px",border:`1px solid ${my?C.greenLt:C.border}`,marginBottom:4,display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12}}><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontWeight:m.winner===m.player1?700:400,color:m.winner===m.player1?C.greenLt:C.text}}>{m.player1}</span><span style={{fontWeight:700}}>{m.p1Total??""}</span></div><div style={{display:"flex",justifyContent:"space-between",marginTop:2}}><span style={{fontWeight:m.winner===m.player2?700:400,color:m.winner===m.player2?C.greenLt:C.text}}>{m.player2}</span><span style={{fontWeight:700}}>{m.p2Total??""}</span></div></div><div style={{textAlign:"right",marginLeft:12,minWidth:70}}><div style={{fontSize:10,color:C.muted}}>{m.course||"TBD"}</div>{m.status==="complete"?<div style={{fontWeight:700,fontSize:11,color:m.winner==="Tie"?C.muted:C.gold,marginTop:2}}>{m.winner==="Tie"?"Tie":m.winner}</div>:my?<button onClick={()=>playLeagueMatch(m.id)} style={{...btnS(true),padding:"4px 10px",fontSize:10,marginTop:2}}>Play</button>:<div style={{fontSize:10,color:C.muted,marginTop:2}}>Pending</div>}</div></div>);})}</div>);})}</div>}

      {/* Bracket */}
      {leagueView==="bracket"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
        {(curLeague.status==="playoffs"||curLeague.status==="complete")&&poMatches.length>0?
          poMatches.sort((a,b)=>{const o={QF:0,SF:1,F:2};return(o[a.roundType]||0)-(o[b.roundType]||0);}).map(m=>{const my=m.player1===me||m.player2===me;const isF=m.roundType==="F";return(<div key={m.id} style={{background:isF?"linear-gradient(135deg,#2a1a00,#1a0f00)":C.card,borderRadius:8,padding:12,border:`1px solid ${isF?"#d4b84a":C.border}`}}><div style={{fontSize:10,color:isF?C.gold:C.muted,marginBottom:4,fontWeight:600}}>{m.roundType==="QF"?"Quarterfinal":m.roundType==="SF"?"Semifinal":"🏆 Championship"}{m.course?` · ${m.course}`:""}</div>{m.player1&&<div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontWeight:m.winner===m.player1?700:400,color:m.winner===m.player1?C.greenLt:C.text}}>{m.player1}</span><span style={{fontWeight:700}}>{m.p1Total??""}</span></div>}{m.player2&&<div style={{display:"flex",justifyContent:"space-between",marginTop:2}}><span style={{fontWeight:m.winner===m.player2?700:400,color:m.winner===m.player2?C.greenLt:C.text}}>{m.player2}</span><span style={{fontWeight:700}}>{m.p2Total??""}</span></div>}{m.status==="complete"&&m.winner&&<div style={{textAlign:"right",fontSize:10,color:C.gold,marginTop:4}}>Winner: {m.winner}</div>}{m.status!=="complete"&&my&&m.player1&&m.player2&&<button onClick={()=>playLeagueMatch(m.id)} style={{...btnS(true),width:"100%",padding:10,marginTop:8,fontSize:12}}>⛳ Play</button>}{m.status!=="complete"&&(!m.player1||!m.player2)&&<div style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:4}}>Awaiting results...</div>}</div>);})
