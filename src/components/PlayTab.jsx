      ...
      {selCourse&&playMode==="review"&&<>
        ...
        {(()=>{
          ...
          return <>
            ...
            {roundPlayers.map(p=>{const sc=allScores[p]||Array(18).fill(null);const tot=sc.slice(startIdx,endIdx).reduce((s,v)=>s+(v||0),0);const hio=countHIO(sc.slice(startIdx,endIdx));return<div key={p} style={{background:C.card,borderRadius:8,padding:10,border:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontWeight:700}}>{p}</div>{useHdcp&&hdcps[p]&&<div style={{fontSize:10,color:C.blue}}>HDCP: {hdcps[p]} · Adj: {tot-hdcps[p]}</div>}</div><div style={{textAlign:"right"}}><div style={{fontSize:20,fontWeight:700}}>{tot} <RelPar s={tot} p={playedPar}/></div>{hio>0&&<div style={{fontSize:10,color:"#ff6b00"}}>🎯 {hio} hole-in-one{hio>1?"s":""}</div>}</div></div>;})}
          </>;
        })()}        <div style={{display:"flex",gap:8}}>
