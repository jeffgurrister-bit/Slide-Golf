...
import { calcPar, fmtR, fmtRange, calcNeed, scoreName, countHIO, RelPar, DIFFICULTY_ORDER, relTime } from "../utils/helpers.jsx";
...
export default function PlayTab({
  ...
  nine, setNine, setQuickScore, quickPutts, setQuickPutts,
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      {/* Course selection */}
      {!selCourse&&<><h2 style={{margin:0,fontSize:18}}>Select Course</h2>{pgaThisWeek&&<button onClick={()=>{setShowTourney(true);setTab("home");}} style={{...btnS(false),width:"100%",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",background:"linear-gradient(135deg,#1a2a4a,#2a3a5a)",border:"1px solid #3a5a8a"}}><span style={{fontWeight:600,color:C.blue}}>📺 {pgaThisWeek.tournament}</span><span style={{fontSize:11,opacity:0.7,color:C.blue}}>Tournament</span></button>}{allCourses.slice().sort((a,b)=>{const hasMust=c=>c.holes.some(h=>h.mustHit);if(hasMust(a)&&!hasMust(b))return -1;if(!hasMust(a)&&hasMust(b))return 1;const da=DIFFICULTY_ORDER[a.level]??99;const db=DIFFICULTY_ORDER[b.level]??99;if(da!==db)return da-db;return a.name.localeCompare(b.name);}).map(c=>{const isTest=c.holes.some(h=>h.mustHit);const creatorLine=c.generated?[c.createdBy&&`by ${c.createdBy}`,c.createdAt&&relTime(c.createdAt)].filter(Boolean).join(" · "):"";return (<button key={c.id} onClick={()=>{setSelCourse(c);setRoundPlayers([]);setAllScores({});setAllShotLogs({});setPlayMode("setup");setActiveTourney(null);}} style={{...btnS(false),width:"100%",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",...(isTest?{background:"linear-gradient(135deg,#3a2a4a,#2a1a3a)",border:"1px solid #8a5acc"}:{})}}><span style={{fontWeight:600,display:"flex",flexDirection:"column",gap:2,minWidth:0}}><span style={{display:"flex",alignItems:"center",gap:8}}>{isTest&&<span style={{background:"#8a5acc",color:"#fff",fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:4,letterSpacing:0.5}}>🧪 NEW</span>}{c.name}</span>{creatorLine&&<span style={{fontSize:9,color:C.muted,fontWeight:400}}>{creatorLine}</span>}</span><span style={{fontSize:11,opacity:0.7}}>Par {c.holes.reduce((s,h)=>s+h.par,0)} · {c.level}</span></button>);})}</>}
      ...
      {selCourse&&playMode==="setup"&&<>
        ...
        {/* 9/18 Hole Toggle */}
        {!activeLeagueMatch&&<div style={{display:"flex",gap:8}}>
          <button onClick={()=>setHoleCount(18)} style={{flex:1,padding:10,borderRadius:8,border:holeCount===18?`2px solid ${C.greenLt}`:`1px solid ${C.border}`,background:holeCount===18?C.accent:C.card,color:holeCount===18?C.white:C.muted,cursor:"pointer",fontSize:13,fontWeight:600}}>18 Holes</button>
          <button onClick={()=>setHoleCount(9)} style={{flex:1,padding:10,borderRadius:8,border:holeCount===9?`2px solid ${C.greenLt}`:`1px solid ${C.border}`,background:holeCount===9?C.accent:C.card,color:holeCount===9?C.white:C.muted,cursor:"pointer",fontSize:13,fontWeight:600}}>9 Holes</button>
        </div>}
        {holeCount===9&&<div style={{display:"flex",gap:8}}>
          <button onClick={()=>setNineType("front")} style={{flex:1,padding:8,borderRadius:8,border:nineType==="front"?`2px solid ${C.blue}`:`1px solid ${C.border}`,background:nineType==="front"?"rgba(138,180,248,0.1)":C.card,color:nineType==="front"?C.blue:C.muted,cursor:"pointer",fontSize:12,fontWeight:600}}>Front 9 (1-9)</button>
          <button onClick={()=>setNineType("back")} style={{flex:1,padding:8,borderRadius:8,border:nineType==="back"?`2px solid ${C.blue}`:`1px solid ${C.border}`,background:nineType==="back"?"rgba(138,180,248,0.1)":C.card,color:nineType==="back"?C.blue:C.muted,cursor:"pointer",fontSize:12,fontWeight:600}}>Back 9 (10-18)</button>
        </div>}
      ...
      {selCourse&&playMode==="holes"&&curHD&&<>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",fontSize:10,padding:"2px 0"}}>
              {totals.map(({p, tot, parSoFar, diff}) => (
                <div key={p} style={{background:C.card2,padding:"3px 8px",borderRadius:6,border:`1px solid ${C.border}`,display:"flex",gap:5,alignItems:"center"}}>
                  ...
                  <span style={{fontWeight:700,color:diff<0?C.red:diff>0?C.greenLt:C.muted}}>{diff===0?"E":diff>0?`+${diff}`:diff}</span>
                </div>
              ))}
            </div>
          );
        })()}
        <div style={{background:"linear-gradient(135deg,rgba(74,170,74,0.12),rgba(74,170,74,0.04))",border:`1px solid ${C.greenLt}`,borderRadius:12,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><h2 style={{margin:0,fontSize:22,fontWeight:800,letterSpacing:0.5}}>Hole {curHole+1} <span style={{fontWeight:400,fontSize:13,color:C.muted}}>of {holeCount===9?(nineType==="back"?"18":"9"):"18"}</span></h2><div style={{fontSize:13,color:C.text,marginTop:2}}><strong style={{color:C.greenLt}}>Par {curHD.par}</strong> <span style={{color:C.muted}}>·</span> <strong>{fmtR(curHD.range)}</strong>{holeCount===9&&<span style={{color:C.blue,marginLeft:6,fontSize:11}}>{nineType==="front"?"Front 9":"Back 9"}</span>}{curHD.mustHit&&<span style={{marginLeft:8,background:"#8a5acc",color:"#fff",padding:"2px 8px",borderRadius:4,fontSize:11,fontWeight:700}}>MUST HIT {curHD.mustHit}</span>}</div></div><button onClick={()=>setShowScorecard(s=>!s)} style={{...btnS(false),padding:"4px 10px",fontSize:11}}>📋</button></div>
        ...
        {roundPlayers.map((p,pIdx)=>{
          ...
          return<div key={p} style={{background:C.card,borderRadius:12,padding:12,border:`1px solid ${hs.done?C.greenLt:C.border}`,opacity:hs.done?0.7:1}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              ...
              :need?<div style={{fontSize:17,color:need.dir==="add"?C.greenLt:C.blue,fontWeight:800,letterSpacing:0.3}}>Need <span style={{fontSize:20}}>{need.lo===need.hi?need.lo:`${need.lo}-${need.hi}`}</span> <span style={{fontSize:11,fontWeight:500,color:C.muted}}>({need.dir==="add"?"slide up":"slide back"})</span></div>
              :hs.onGreen?<div style={{fontSize:14,color:C.greenLt,fontWeight:700}}>🏁 On the Green</div>:null}
            </div>
            ...
            {!hs.done&&isMyTurn&&<>
              {!hs.onGreen&&<div style={{marginBottom:4}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  ...
                  <button onClick={()=>recordShot(p,"HOLEOUT")} style={{flex:1,height:40,borderRadius:8,border:"1px solid rgba(255,107,0,0.4)",background:"rgba(255,107,0,0.1)",color:"#ff6b00",cursor:"pointer",fontSize:13,fontWeight:700}}>🎯 Hole Out</button>
                </div>
              </div>}
        ...
        <div style={{display:"flex",gap:8}}>
          {curHole>(holeCount===9&&nineType==="back"?9:0)&&<button onClick={goToPrevHole} style={{...btnS(false),padding:"10px 16px",fontSize:12}}>← Hole {curHole}</button>}
          <button onClick={finishHole} style={{...btnS(true),flex:1,padding:14,fontSize:15}}>{curHole<(holeCount===9?(nineType==="back"?17:8):17)?`Hole ${curHole+1} Done →`:"Finish Round"}</button>
        </div>
      </>}
      ...
      {selCourse&&playMode==="quick"&&<>
        ...
        {(()=>{
          ...
          {roundPlayers.map(p=>{const sc=allScores[p]||Array(18).fill(null);return<tr key={p} style={{borderTop:`1px solid ${C.border}`}}><td style={{padding:"3px 6px",fontWeight:600,fontSize:9}}>{p}</td>{selCourse.holes.slice(qStart,qEnd).map((h,i)=>{const idx=qStart+i;return<td key={h.num} style={{padding:"2px"}}><input value={sc[idx]??""} onChange={e=>setQuickScore(p,idx,e.target.value)} style={{...smallInput,width:"100%",fontSize:11,color:sc[idx]!=null&&sc[idx]<h.par?C.red:sc[idx]!=null&&sc[idx]>h.par?C.greenLt:C.text}}/></td>;})} <td style={{textAlign:"center",fontWeight:700,fontSize:10}}>{sc.slice(qStart,qEnd).reduce((s,v)=>s+(v||0),0)||"-"}</td></tr>;})}
          </tbody></table></div>;
        })()}
      ...
      {selCourse&&playMode==="review"&&<>
        <h2 style={{margin:0,fontSize:18}}>📋 Round Review {holeCount===9&&<span style={{fontSize:13,fontWeight:400,color:C.muted}}>({nineType==="front"?"Front 9":"Back 9"})</span>}</h2>
        {(()=>{
          const rvNines=holeCount===9?(nineType==="back"?[9]:[0]):[0,9];
          const startIdx=holeCount===9?(nineType==="back"?9:0):0;
          const endIdx=startIdx+holeCount;
          const playedPar=selCourse.holes.slice(startIdx,endIdx).reduce((s,h)=>s+h.par,0);
          return <>
            <div style={{background:C.card,borderRadius:12,padding:12,border:`1px solid ${C.border}`,overflowX:"auto"}}>{rvNines.map(start=>(<table key={start} style={{width:"100%",borderCollapse:"collapse",fontSize:9,marginBottom:rvNines.length>1&&start===0?6:0,minWidth:420}}><thead><tr style={{background:C.accent}}><th style={{padding:"4px 6px",textAlign:"left",minWidth:50}}>HOLE</th>{selCourse.holes.slice(start,start+9).map(h=><th key={h.num} style={{padding:"3px 2px",textAlign:"center",minWidth:24}}>{h.num}</th>)}<th style={{padding:"3px 4px",textAlign:"center",minWidth:30}}>{start===0?"OUT":"IN"}</th>{((start===9&&holeCount===18)||(holeCount===9))&&holeCount===18&&<th style={{padding:"3px 4px",textAlign:"center",minWidth:30}}>TOT</th>}</tr></thead><tbody>{selCourse.holes.some(h=>h.mustHit)&&<tr style={{background:"rgba(138,90,204,0.12)"}}><td style={{padding:"2px 6px",fontWeight:700,color:"#c79aff",fontSize:9}}>MUST</td>{selCourse.holes.slice(start,start+9).map(h=><td key={h.num} style={{textAlign:"center",fontSize:9,color:"#c79aff",fontWeight:700}}>{h.mustHit||"—"}</td>)}<td/>{start===9&&holeCount===18&&<td/>}</tr>}<tr><td style={{padding:"2px 6px",fontWeight:600}}>PAR</td>{selCourse.holes.slice(start,start+9).map(h=><td key={h.num} style={{textAlign:"center"}}>{h.par}</td>)}<td style={{textAlign:"center",fontWeight:700}}>{calcPar(selCourse.holes,start,start+9)}</td>{start===9&&holeCount===18&&<td style={{textAlign:"center",fontWeight:700,color:C.greenLt}}>{playedPar}</td>}</tr>{roundPlayers.map(p=>{const sc=allScores[p]||Array(18).fill(null);return<tr key={p} style={{borderTop:`1px solid ${C.border}`}}><td style={{padding:"2px 6px",fontWeight:600,fontSize:8}}>{p}</td>{selCourse.holes.slice(start,start+9).map((h,i)=>{const idx=start+i;const v=sc[idx];return<td key={h.num} style={{textAlign:"center",fontWeight:700,color:v===1?"#ff6b00":v!=null&&v<h.par?C.red:v!=null&&v>h.par?C.greenLt:v!=null?C.text:C.muted}}>{v??"-"}</td>;})}<td style={{textAlign:"center",fontWeight:700}}>{sc.slice(start,start+9).reduce((s,v)=>s+(v||0),0)||"-"}</td>{start===9&&holeCount===18&&<td style={{textAlign:"center",fontWeight:700,color:C.greenLt}}>{sc.reduce((s,v)=>s+(v||0),0)||"-"}</td>}</tr>;})}</tbody></table>))}</div>
            {roundPlayers.map(p=>{const sc=allScores[p]||Array(18).fill(null);const tot=sc.slice(startIdx,endIdx).reduce((s,v)=>s+(v||0),0);const hio=countHIO(sc.slice(startIdx,endIdx));const hasShots=(allShotLogs[p]||[]).some(h=>h&&h.length>0);return<div key={p} style={{background:C.card,borderRadius:8,padding:10,border:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap"}}><div><div style={{fontWeight:700}}>{p}</div>{useHdcp&&hdcps[p]&&<div style={{fontSize:10,color:C.blue}}>HDCP: {hdcps[p]} · Adj: {tot-hdcps[p]}</div>}</div>{!hasShots&&<div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:C.muted}}><span>Putts:</span><input value={quickPutts?.[p]??""} onChange={e=>{const v=e.target.value.replace(/[^0-9]/g,"");setQuickPutts(prev=>({...(prev||{}),[p]:v}));}} placeholder="—" inputMode="numeric" style={{padding:"4px 6px",border:`1px solid ${C.border}`,borderRadius:4,background:C.card2,color:C.text,fontSize:12,width:46,textAlign:"center"}}/></div>}<div style={{textAlign:"right"}}><div style={{fontSize:20,fontWeight:700}}>{tot} <RelPar s={tot} p={playedPar}/></div>{hio>0&&<div style={{fontSize:10,color:"#ff6b00"}}>🎯 {hio} hole-in-one{hio>1?"s":""}</div>}</div></div>;})}
          </>;
        })()}        <div style={{display:"flex",gap:8}}>
