...
// League convention: under-par = red, over-par = green (intentional flip
// from standard golf colors — easier for the group to scan their cards).
// Albatross/eagle/HIO keep their celebratory gold/orange.
export function scoreName(s,p,isHoleOut=false){
  ...
  if(d===-1)return{l:isHoleOut?"Birdie! (Hole Out)":"Birdie!",c:"#ef4444",e:"🐦"};
  if(d===0)return{l:isHoleOut?"Par (Hole Out)":"Par",c:"#aaa",e:"👍"};
  if(d===1)return{l:"Bogey",c:"#22c55e",e:""};
  if(d===2)return{l:"Dbl Bogey",c:"#16a34a",e:""};
